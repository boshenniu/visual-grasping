...
class GripperController:
    def __init__(self, router, router_real_time, proportional_gain = 0.5):
        """
            GripperController class constructor.
        ...
        self.set_low_level_servoing_mode()
    ...
    def Goto(self, target_position):
        # Change servoing mode to low level servoing
        self.set_low_level_servoing_mode()
        ...
        while True:
            try:
                ...
                cur_pos = base_feedback.interconnect.gripper_feedback.motor[0].position
                position_error = target_position - cur_pos
                
                if pos_his[t] != -1 and abs(cur_pos - pos_his[t]) < 5:
                    self.motorcmd.velocity = 0
                    self.base_cyclic.Refresh(self.base_command)
                    print("Force limit triggered")
                    print("waiting 3s to check")
                    time.sleep(3)
                    break
...
def main():
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        with utilities.DeviceConnection.createUdpConnection(args) as router_real_time:
            ...
            kbhit = KBHit()
            example = GripperController(router, router_real_time, args.proportional_gain)
